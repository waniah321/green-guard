...
// --- Migrated Nodemailer and News API Endpoints from original server.js ---
...
// Database Auto-Creation and Sync Initialization
const { Client } = require('pg');

async function initializeDatabase() {
  const dbName = process.env.DB_NAME || 'green-guard';
  console.log(`Checking PostgreSQL connection & ensuring database "${dbName}" exists...`);
  ...
  try {
    ...
    // Check if green-guard database exists
    ...
    console.error('⚠️ Warning: Failed to check/create database using "postgres" default DB:', err.message);
    console.log('Will attempt to connect directly using Sequelize...');
  } finally {
    try {
      await client.end();
    } catch (e) {}
  ...
  // Now sync tables with Sequelize
  try {
    await sequelize.sync({ alter: true });
    console.log('📦 PostgreSQL Database synced successfully.');
    app.listen(PORT, () => {
      console.log(`🚀 GreenGuard unified backend server running on port ${PORT}`);
    });
  } catch (err) {
    console.error('❌ Failed to sync database or connect to PostgreSQL:', err);
  }
}
